...
extension UITableViewController {
	...
	private func moveRowsUpToEarliestRow(
		...
		// Generate the new `indexPaths` for the recursive call.
		...
		let newFirstIndexPath = IndexPath(
			row: firstIndexPath.row + 1,
			section: firstIndexPath.section)
		
		// Recursive call.
		moveRowsUpToEarliestRow(
			from: newSortedIndexPaths,
			startingAt: newFirstIndexPath,
			completion: completion)
	}

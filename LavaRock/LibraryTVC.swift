...
class LibraryTVC: UITableViewController {
	final var onscreenRowIdentifiers: [AnyHashable] = []
	...
	final func applyRowIdentifiers(
		_ newIdentifiers: [AnyHashable],
		runningBeforeContinuation: (() -> Void)? = nil
	) async -> Bool {
		await withCheckedContinuation { continuation in
			_applyRowIdentifiers(newIdentifiers) { shouldContinue in
				continuation.resume(returning: shouldContinue)
			}
			runningBeforeContinuation?()
		}
	}
	private func _applyRowIdentifiers(
		_ newIdentifiers: [AnyHashable],
		...
		isAnimatingRows += 1
		let old = onscreenRowIdentifiers
		onscreenRowIdentifiers = newIdentifiers
		tableView.performUpdatesFromRowIdentifiers(old: old, new: newIdentifiers) {
			// Completion handler

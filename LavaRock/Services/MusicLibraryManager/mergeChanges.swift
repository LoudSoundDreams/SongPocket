...
extension MusicLibraryManager {
	...
	final func mergeChanges(toMatch freshSongFiles: [SongFile]) {
		...
		let existingCollections = Collection.allFetched(via: context) // Order matters, because we'll try to add new `Album`s to the first `Collection` with a matching title.
